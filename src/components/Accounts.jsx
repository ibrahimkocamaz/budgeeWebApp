    ...
    return (
        <div className="accounts-container fade-in">
                                    ...
                                    <div className="account-card-header">
                                        <div className="header-left">
                                            ...
                                            <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                                                ...
                                                {account.due_date && account.type === 'Card' && (
                                                    <div className={`account-due-badge ${(() => {
                                                        const today = new Date();
                                                        const due = parseInt(account.due_date);
                                                        ...
                                                        {(() => {
                                                            ...
                                                            const currentDay = today.getDate();
                                                            const diff = due - currentDay;
                                                            if (diff === 0) return t('accounts.dueToday') || 'Due Today';
                                                            return t('accounts.dueOn', { date: account.due_date }) || `Day ${account.due_date}`;
                                                        })()}
                                                    </div>
                                                )}
                                            </div>
                                                        ...
                                                        <div className="credit-bar-wrapper">
                                                            <div className="credit-bar-track">
                                                                <div 
                                                                    ...
                                                                    style={{ 
                                                                        width: `${Math.min(100, (Math.abs(account.balance) / account.credit_limit) * 100)}%`,
                                                                        backgroundColor: (Math.abs(account.balance) / account.credit_limit) > 0.8 ? 'var(--color-accent-red)' : 'var(--color-brand)'
                                                                    }}
                                                                ></div>
                                                            </div>
                                                        </div>
                                                        ...
                                                        <div className="cc-footer">
                                                            <span className={`cc-due ${(() => {
                                                                ...
                                                                {(() => {
                                                                    ...
                                                                    const diff = due - currentDay;
                                                                    if (diff === 0) return t('accounts.dueToday') || 'Due Today';
                                                                    if (diff === 1) return t('accounts.dueTomorrow') || 'Due Tomorrow';
                                                                    if (diff > 1 && diff <= 7) return t('accounts.dueInDays', { count: diff }) || `In ${diff} days`;
                                                                    return t('accounts.dueOn', { date: account.due_date }) || `Due on ${account.due_date}`;
                                                                })()}
                                                            </span>
                                                        </div>
                                                    )}
            ...
            <style>{`
                ...
                .account-due-badge.urgent {
                    background: rgba(229, 57, 53, 0.2);
                    color: #ef5350;
                }

                .account-due-badge.soon {
