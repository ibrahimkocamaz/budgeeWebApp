...
const AddTransaction = () => {
  ...
  const { addTransaction, addTransactions, transferMoney } = useTransactions();
  ...
  const handleSubmit = async (e) => {
    ...
    transactions.forEach(tx => {
      if (tx.type === 'transfer') {
        ...
        for (let i = 0; i < count; i++) {
          let txDate = new Date(tx.date + 'T12:00:00'); // Use noon to avoid TZ issues
          if (i > 0) {
    ...
    if (batchTransactions.length > 0) {
      promises.push(addTransactions(batchTransactions));
    }
  ...
  return (
    <div className="add-transaction-container">
            ...
            <div key={tx.id} className="transaction-card">
              ...
              <div className="card-header-row">
                <div className="segmented-toggle">
                  ...
                  <button
                    type="button"
                    className={`toggle-option ${tx.type === 'transfer' ? 'active transfer' : ''}`}
                    onClick={() => handleChange(tx.id, 'type', 'transfer')}
                  >
                    {t('addTransaction.transfer') || 'Transfer'}
                  </button>
                  <button
                    type="button"
                ...
                {tx.type === 'transfer' ? (
                  <>
                    <div className="detail-item">
                      <label>{t('addTransaction.fromAccount') || 'From Account'}</label>
                    ...
                    <div className="detail-item">
                      <label>{t('addTransaction.toAccount') || 'To Account'}</label>
                      <div className="select-wrapper">
                        <Wallet size={16} className="field-icon" />
                        <select
                          value={tx.toAccountId || ''}
                          onChange={(e) => handleChange(tx.id, 'toAccountId', e.target.value)}
                        >
                          <option value="" disabled>{t('addTransaction.selectAccount') || 'Select Account'}</option>
                          {accounts.filter(a => a.id !== tx.accountId).map(acc => (
                            <option key={acc.id} value={acc.id}>{acc.name}</option>
                          ))}
                        </select>
                      </div>
      ...
      <style>{`
                ...
                .toggle-option.active.transfer { background-color: #9c27b0; }
